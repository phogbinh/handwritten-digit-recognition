...
for train_round_i = 1:nn.TRAIN_ROUNDS_N
    for mini_batch_i = 1:(TRAIN_DATA_N / nn.MINI_BATCH_LENGTH)
        for l_i = 2:LAYERS_N
            layers(l_i).dcdb_cum = zeros( size( layers(l_i).b ) );
            layers(l_i).dcdw_cum = zeros( size( layers(l_i).w ) );
        end
        ...
        for train_data_i = start_i:end_i
            ...
            % feedfoward
            layers(1).y = transpose( TRAIN_IN(train_data_i, :) );
            for l_i = 2:LAYERS_N
            ...
            % backpropagation
            dcdb = ( 2 * ( layers(LAYERS_N).y - desired_output_layer ) ) .* df_dz( layers(LAYERS_N).z );
            dcdw = dcdb * transpose( layers(LAYERS_N - 1).y );
            layers(LAYERS_N).dcdb_cum = layers(LAYERS_N).dcdb_cum + dcdb;
            layers(LAYERS_N).dcdw_cum = layers(LAYERS_N).dcdw_cum + dcdw;
            for l_i = (LAYERS_N - 1):-1:2
                dcdb = ( transpose( layers(l_i + 1).w ) * dcdb ) .* df_dz( layers(l_i).z );
                dcdw = dcdb * transpose( layers(l_i - 1).y );
                layers(l_i).dcdb_cum = layers(l_i).dcdb_cum + dcdb;
                layers(l_i).dcdw_cum = layers(l_i).dcdw_cum + dcdw;
            end
        end

        for l_i = 2:LAYERS_N
            layers(l_i).b = layers(l_i).b - layers(l_i).dcdb_cum ./ nn.MINI_BATCH_LENGTH * nn.LEARNING_RATE;
            layers(l_i).w = layers(l_i).w - layers(l_i).dcdw_cum ./ nn.MINI_BATCH_LENGTH * nn.LEARNING_RATE;
        end
    end
end
